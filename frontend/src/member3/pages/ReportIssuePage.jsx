...
function mapAxiosErrorToFormErrors(err) {
  if (!err.response) {
    const hint = err?.message || "Network error";
    return {
      general: `${hint}. Start the Java backend, keep "npm run dev" running, and ensure the API port in backend/.dev-server-port matches the server (e.g. Uniflow log line with http://127.0.0.1:...).`,
    };
  }
  const { status, data: raw } = err.response;
  let data = raw;
  if (typeof raw === "string") {
    ...
    if (t.startsWith("{")) {
      try {
        ...
        data = { message: t.slice(0, 400) };
      }
    } else {
      return {
        general: `Server returned HTTP ${status} (non-JSON). ${t.slice(0, 200)}`,
      };
    }
  }
  if (!data || typeof data !== "object") {
    return { general: `Request failed (HTTP ${status})` };
  }
  if (data.errors && typeof data.errors === "object") {
    return {
      ...data.errors,
      general: data.message || data.error || `Validation failed (HTTP ${status})`,
    };
  }
  return {
    general: data.message || data.error || `Request failed (HTTP ${status})`,
  };
}

/** Cached display; prefer /auth/me so the form matches the logged-in JWT, not stale localStorage. */
function loadReporterFromSession() {
...
/** JWT `sub` is the login email (matches backend JwtUtil). Used if /auth/me fails. */
...
function buildInitialForm() {
  const { name, email } = loadReporterFromSession();
  return {
    ...emptyIssueFields,
    name: name || "—",
    email: email || "—",
...
export default function ReportIssuePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const jwtEmail = emailFromJwt(tok);
        const stored = loadReporterFromSession();
        setForm((prev) => ({
          ...prev,
          email: (jwtEmail || stored.email || "").trim() || prev.email,
          name: (stored.name || "").trim() || prev.name,
        }));
        if (jwtEmail) localStorage.setItem("email", jwtEmail);
      }
    })();
    return () => {
      cancelled = true;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(`/m3/issues/${createdIssue.id}`);
    } catch (err) {
      setErrors(mapAxiosErrorToFormErrors(err));
    }
  };
            ...
            <div className="side-panel">
              <section className="premium-actions">
                ...
                <div className="action-links">
                  <Link to="/m3" className="action-link">
                    <div>
                  ...
                  <Link to="/m3/my-reports" className="action-link">
                    <div>
                  ...
                  <Link to="/m3/featured" className="action-link">
                    <div>
